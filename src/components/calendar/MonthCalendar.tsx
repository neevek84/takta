 ...
 * défaut —, contre 3,46 en Neutre clair et 4,90 en KreativPM, quand le projet
 * s'impose `MIN_LIGHTNESS_GAP` = 4 pour qu'un état se lise sans distinguer les
 * teintes. Aucune valeur de bordure ne pouvait le sauver : elle aurait à tenir
 * contre onze fonds inconnus d'avance, dont un ambre et six teintes
 * catégorielles. C'est la leçon du lot 1f, qui a remplacé les hachures du
 * prévisionnel par une horloge pour exactement cette raison.
 *
 ...
 *
 * Posé en absolu, comme l'aplat : la ligne du numéro du jour porte déjà le
 * losange d'occupation et l'horloge du prévisionnel, et un troisième glyphe
 * dans ce flux ferait tomber le budget des sept colonnes à 375 points.
 *
...
function CoinEclate({ date }: { date: string }) {
  return (
    <svg
      aria-hidden="true"
      data-testid={`eclatement-${date}`}
      width={COTE_DU_COIN}
      height={COTE_DU_COIN}
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        {/* Après l'aplat, jamais avant : sans z-index, c'est l'ordre du
            ...
            qu'il traverse. */}
        {etat.kind === 'LIBRE' && etat.eclatee && <CoinEclate date={jour.date} />}
